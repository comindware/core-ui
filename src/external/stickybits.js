...
*/
/*eslint-ignore*/ class Stickybits {
    ...
    */ this.props.positionVal =
            this.definePosition() || 'fixed';
        this.instances = [];
  ...
  */
    computeScrollOffsets(item) {
        ...
        const stickyStart = isCustom ? this.getTopPosition(item.el) - scrollElOffset : this.getTopPosition(item.el);
        const parentMargin = isCustom ? this.getTopPosition(parent) - scrollElOffset : this.getTopPosition(parent);
        const stickyChangeOffset = p.customStickyChangeNumber !== null ? p.customStickyChangeNumber : el.offsetHeight;
        const parentBottom = parentMargin + parent.offsetHeight;
        it.offset = scrollElOffset + p.stickyBitStickyOffset;
        it.stickyStart = isTop ? (isCustom ? stickyStart : stickyStart - it.offset) : 0;
        it.stickyChange = it.stickyStart + stickyChangeOffset;
        it.stickyStop = isTop ? parentBottom - (isCustom ? el.offsetHeight : el.offsetHeight + it.offset) : parentBottom - window.innerHeight;
        return it;
    }
    /*
    toggleClasses ⚖️
  ...
  */ manageState(item) {
      ...
      - or stub rAF
    */
        const rAFStub = function rAFDummy(f) {
            f();
        };
        const rAF = !this.isWin
            ? rAFStub
            : window.requestAnimationFrame || window.mozRequestAnimationFrame || window.webkitRequestAnimationFrame || window.msRequestAnimationFrame || rAFStub;
        /*
      define scroll vars
      ---
      - scroll
