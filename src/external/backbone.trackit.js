...
(function () {
  ...
  var getPromptConfig = function(fnName) {
    var config, args = _.rest(arguments);
    ...
    _.find(unsavedModels, function(model) {
  ...
  Backbone.History.prototype.navigate = _.wrap(Backbone.History.prototype.navigate, function(oldNav, fragment, options) {
    var config = getPromptConfig('unloadRouterPrompt', fragment, options);
    if (config) {
      var confirm = config.confirm || window.confirm;
      $.when(confirm(config.prompt)).done(function (doNavigate) {
  ...
  window.onbeforeunload = function(e) {
    var config = getPromptConfig('unloadWindowPrompt', e);
