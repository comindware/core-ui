...
const ContextModel = Backbone.Model.extend({
    ...
    populateChildren() {
        if (!this.get('hasChildren') || !this.get('context')) {
            return;
        }
    ...
    selectPath(selectedItem) {
        ...
        this.get('children').models.forEach(child => {
            if (selectedItem.indexOf(child.get('id')) > -1) {
    ...
    __collapse() {
        ...
        this.get('children').models.forEach(model => {
